...
import type {
  ...
  CanvasSection,
  TrafficLightColor,
} from '../../../../../shared/mooring-line.model';
...
function computeTL(lineIds: string[], lines: MooringLine[]): TrafficLightColor {
  ...
  const tls = lineIds.map((id) => {
    ...
    if (l.status === 'RETIRED' || l.status === 'OUT_OF_SERVICE')
      return 'GREY' as TrafficLightColor;
    ...
    const hasRed = inspections.some(
      (i) =>
        i.actionRequired === 'RETIRE' ||
        i.overallCondition === 'RETIRE' ||
        i.actionRequired === 'EARLY_REINSPECTION' ||
        i.wearZoneStatus === 'CRITICAL',
    );
    if (hasRed) return 'RED' as TrafficLightColor;
    const hasAmber = inspections.some(
      (i) =>
        i.actionRequired === 'MONITOR' ||
        i.overallCondition === 'MARGINAL' ||
    ...
    <div class="rounded-xl border border-base-300 bg-white overflow-hidden h-full flex flex-col">
      ...
      <svg
        ...
        [attr.preserveAspectRatio]="
          section() === 'bow' ? 'xMidYMax meet' : 'xMidYMid meet'
        "
      >
        <!-- BOW background -->
        @if (section() === 'bow') {
          <defs>
            <clipPath id="hullClipRO">
              <path
                d="M20,515 C20,316.973 171.417,77.055 300,15 C431.868,79.935 583.658,313.463 583.658,515 Z"
              />
            </clipPath>
          </defs>
          <path
          ...
          <g clip-path="url(#hullClipRO)">
            <path
              d="M120,223 C148,150 238,40 290,32 C296,24 306,24 312,32 C364,40 454,150 482,223 Z"
            ...
            />
            <path
              d="M570,506 L30,506 C33,399 63,313 111,240 L489,240 C541,321 568,409 570,506 Z"
              fill="rgba(255,255,255,0.88)"
              stroke="#0284c7"
              stroke-width="1.5"
              stroke-linejoin="round"
        ...
        @if (section() === 'aft') {
          <path
            d="M18,10 L582,10 L519,510 L81,510 Z"
            fill="#bae6fd"
            stroke="#0284c7"
            stroke-width="2.5"
          />
          <path
            d="M30,22 L570,22 L540,235 L60,235 Z"
            fill="rgba(255,255,255,0.88)"
            stroke="#0284c7"
            stroke-width="1.5"
            stroke-linejoin="round"
          ...
          />
          <path
            d="M60,253 L540,253 L508,500 L92,500 Z"
            fill="rgba(255,255,255,0.88)"
            stroke="#0284c7"
            stroke-width="1.5"
            stroke-linejoin="round"
        ...
        @for (item of visibleItems(); track item.id) {
          ...
          >
            <!-- Basket identification label: above icon, not rotated -->
            @if (item.type === 'basket') {
              <text
                x="24"
                y="-4"
              ...
              >
                {{ item.identification || 'identification as per MMP' }}
              </text>
            }
            ...
            <g [attr.transform]="'rotate(' + item.rotation + ',24,24)'">
              @if (item.type === 'basket') {
                ...
                <rect
                  ...
                  rx="10"
                  fill="none"
                  [attr.stroke]="itemBorderColor(item)"
                  stroke-width="2"
              ...
              @if (item.type === 'winch') {
                ...
                <rect
                  ...
                  stroke-dasharray="4,2"
                />
                <!-- Winch identification label: rotates with icon -->
                <text
                  x="24"
                  y="-4"
                  text-anchor="middle"
              ...
              @if (item.type === 'spare-rope') {
                <path
                  d="M 8,16 L 8,36 C 8,39 14,41 22,41 C 30,41 36,39 36,36 L 36,16"
                  fill="#e5e7eb"
                />
                <ellipse
                  cx="22"
            ...
            <!-- Basket + spare-rope cable/hours labels: not rotated -->
            @if (item.type !== 'winch') {
              <text
                x="24"
                y="55"
              ...
              @if (item.type === 'basket' && item.lineIds.length > 1) {
                <text
                  ...
                  paint-order="stroke fill"
                >
                  {{ basketHours(item) }}h tot
                </text>
              }
              @if (item.type === 'spare-rope' && item.lineIds.length === 1) {
                <text
                  x="24"
                  y="65"
                  text-anchor="middle"
...
export class MooringCanvasReadonlyComponent {
  ...
  itemLabel(item: MooringItem): string {
    ...
    if (item.type === 'basket' && item.lineIds.length > 1)
      return `${item.lineIds.length} lines`;
    return this.lines().find((l) => l.id === item.lineIds[0])?.label ?? '+';
  }

  basketHours(item: MooringItem): number {
    return Math.round(
      item.lineIds.reduce((sum, id) => {
        return (
          sum + (this.lines().find((l) => l.id === id)?.totalServiceHours ?? 0)
        );
      }, 0),
    );
  }
  ...
  lineHours(lineId: string): number {
    return Math.round(
      this.lines().find((l) => l.id === lineId)?.totalServiceHours ?? 0,
    );
  }
}
