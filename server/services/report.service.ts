...
export async function getFleetReport() {
  ...
  return ships.map(s => {
    const rotation  = s.rotationLatest?.data  as any
    const mooring   = s.mooringLatest         as any
    const lines: any[] = mooring?.lines ?? []
    ...
    if (rotation?.ports) {
      for (const port of rotation.ports) {
        for (const act of port.activities ?? []) {
          ...
          if (act.speedKts) speeds.push(act.speedKts)
        }
      }
    }
    ...
    const redCount   = lines.filter((l: any) => computeTL(l) === 'RED').length
    const amberCount = lines.filter((l: any) => computeTL(l) === 'AMBER').length
    const greenCount = lines.filter((l: any) => computeTL(l) === 'GREEN').length
...
function computeTL(line: any): 'RED' | 'AMBER' | 'GREEN' | 'GREY' {
